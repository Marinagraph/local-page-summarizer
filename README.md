...
When the selected model advertises an `off` reasoning mode, the extension sends both `reasoning_effort: none` and `enable_thinking: false` regardless of whether the model was loaded manually, selected by exact name, or selected through `auto`. With the default `Max chars` value, automatic selections and Gemma models also raise the effective chunk size to `16000` for contexts of at least 32k or `24000` for contexts of at least 64k. This reduces reasoning tokens and repeated section calls without dropping source text or comments. A manually changed `Max chars` value is never overridden.
...
- On general article/blog/review pages, the extension first tries the bundled Defuddle extractor for cleaner body text, then falls back to the existing selector-based extractor when Defuddle returns too little or suspiciously large content. X and Kakaku use their dedicated collectors without cloning the full page for Defuddle.
...
- On `x.com/{user}/status/{id}` pages, the extension separates the opened post from its conversation replies, excludes nested quoted posts and unrelated recommendation areas, and caches reply elements as they appear while you scroll. Only replies loaded in that open tab are analyzed and saved; it does not request hidden replies from X.
...
- Markdown `Source Text` stores both the extracted page body and every collected comment/review as numbered entries. This includes paginated Danawa opinions, Kakaku reviews, and Kakaku BBS thread openers and replies, so downstream Markdown or JSON analysis can inspect the same raw elements used by the summarizer.
